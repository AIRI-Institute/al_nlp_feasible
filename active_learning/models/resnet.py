...
class ResNet(nn.Module):
    ...
    def forward(self, image, labels):  # torch.Size([64, 1, 28, 28])
        image = image.unsqueeze(1)  # torch.Size([8, 1, 28, 28])
        out = self.activation(
            self.bn1(self.conv1(image))
        )  # torch.Size([64, 64, 28, 28])
        out = self.layer1(out)  # torch.Size([64, 64, 28, 28])
        out = self.layer2(out)  # torch.Size([64, 128, 14, 14])
        out = self.layer3(out)  # torch.Size([64, 128, 14, 14])
        out = self.layer4(out)
        out = F.avg_pool2d(out, 4)  # torch.Size([64, 512, 1, 1])
